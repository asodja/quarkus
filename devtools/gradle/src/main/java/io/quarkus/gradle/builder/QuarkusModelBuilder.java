...
public class QuarkusModelBuilder implements ParameterizedToolingModelBuilder<ModelParameter> {
    ...
    public QuarkusModelBuilder() {
        this.taskName = null;
    }

    public QuarkusModelBuilder(String taskName) {
    ...
    public Object buildAll(String modelName, ModelParameter parameter, Project project) {
        ...
        // quarkusDeployment is resolved twice, once in prepare task and once in actual run.
        // Lets use copy in prepare task to avoid already resolved configuration error.
        final Configuration quarkusDeploymentConfiguration = "quarkusPrepare".equals(taskName)
                ? project.getConfigurations().getByName("quarkusDeployment").copy()
                : project.getConfigurations().getByName("quarkusDeployment");
        ...
        collectFirstMetDeploymentDeps(configuration.getFirstLevelModuleDependencies(), appDependencies,
                quarkusDeploymentConfiguration, visitedDeps);

        final List<Dependency> extensionDependencies = collectExtensionDependencies(project, quarkusDeploymentConfiguration);
